...
@Component({
  ...
  styleUrls: ['./material-usage.component.scss'],
  animations: [
...
export class MaterialUsageComponent implements OnInit, OnDestroy {
  reportModel: MaterialUsageReportModel;
  ...
  columns = [
    ...
    {
      field: 'weightScrapPerc',
      displayName: 'Scrap',
      unit: '%',
    },
  ];

  columnsToDisplayWithExpand = ['expand', 'date', 'material', ...this.columns.map((c) => c.field)];
    ...
    reportFilterSubject$
      .pipe(
        ...
        map((filters) => {
          ...
          return this.http.get(`${Ams.Config.BASE_URL}/_api/reports/materialusage`, {
            params: query,
          });
        }),
        switchMap((source) => source)
      )
      .subscribe((report: MaterialUsageReportModel) => {
