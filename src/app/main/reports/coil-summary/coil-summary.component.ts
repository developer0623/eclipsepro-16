...
@Component({
  selector: 'app-coil-summary',
  templateUrl: './coil-summary.component.html',
  styleUrls: ['./coil-summary.component.scss'],
  animations: [
...
export class CoilSummaryComponent implements OnInit, OnDestroy {
    ...
    reportFilterSubject$
      .pipe(
        ...
        map((filters) => {
          ...
          return this.http.get(`${Ams.Config.BASE_URL}/_api/reports/coilsummary`, {
            params: query,
          });
        }),
  ...
  onChangeDuration(item) {
    ...
    localStorage.setItem('report.coilSummary.Duration', item);
    this.reportFilterChanges$.next({
      duration: item,
      isBack: false,
    });
  ...
  onGetFilterIndex(mainTxt: string, searchTxt: string): boolean {
    ...
    if (!searchTxt) {
      ...
      this.filteredList = this.summaryList.filter((item) => {
        if (this.onGetFilterIndex(item.coil.coilId, realSearchTxt)) {
          ...
          item.usages.some(
            (o) =>
              this.onGetFilterIndex(o.orderCode, realSearchTxt) ||
              this.onGetFilterIndex(o.customerName, realSearchTxt)
          )
        ) {
          return true;
        } else {
