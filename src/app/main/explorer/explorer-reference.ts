...
interface IData {
  ...
  hourOfDay?: number;
  dayOfWeek?: number;
  coilVendor?: { vendorCode: string; vendorName: string };
}
...
export type CrossFilterData = ReturnType<typeof loadCrossFilter>;

export type DimensionIData = Dimension<IData, string | number>;
export type GroupIData = Group<IData, string, string | number>;
...
function loadCrossFilter(
  ...
  const byCoilVendorDimension = crossFilter.dimension((d) => d.coilVendor.vendorCode);
...
export type DeviceCrossFilterData = ReturnType<typeof loadDeviceCrossFilter>;
...
function loadDeviceCrossFilter(
  ...
  const byDayDimension = crossFilter.dimension((d) => d3.timeDay(new Date(d.end)));
  ...
  const byHourOfDayDimension = crossFilter.dimension((d) => d.hourOfDay);
  const byPartGirthDimension = crossFilter.dimension((d) => d.partGirth);
  const byGeometryDimension = crossFilter.dimension((d) => d.geometry);
  ...
  return {
    ...
    byPartGirthDimension: byPartGirthDimension,
    byPartGirthGroup: byPartGirthDimension
      .group()
      .reduce(reduceAdd, reduceRemove, reduceInitial)
      .order(reduceOrderFn),

    byGeometryDimension: byGeometryDimension,
    byGeometryGroup: byGeometryDimension
      .group()
      .reduce(reduceAdd, reduceRemove, reduceInitial)
      .order(reduceOrderFn),
  };
