...
export class DurationDisplayComponent implements OnInit, OnDestroy, OnChanges {
  ...
  @Input() short: boolean; // if true, only show a single unit of time, otherwise show up to two units
  output: string;
  interval;
  ...
  private calculateDateParts() {
    ...
    this.output = this.calculateOutput(months, days, hours, minutes, seconds, this.short);
  }

  private calculateOutput(months, days, hours, minutes, seconds, short: boolean) {
    if (months > 240) {
      //really old, show nothing
      return '';
    }
    if (months > 0) {
    ...
    if (days > 0) {
      return (
        `${days}<span class='time-sign'>D&nbsp;</span>` +
        (short ? '' : `${hours}<span class='time-sign'>H</span>`)
      );
    }
    if (hours > 0) {
      return (
        `${hours}<span class='time-sign'>H&nbsp;</span>` +
        (short ? '' : `${minutes}<span class='time-sign'>M</span>`)
      );
    }
    if (minutes > 0) {
      return (
        `${minutes}<span class='time-sign'>M&nbsp;</span>` +
        (short ? '' : `${seconds}<span class='time-sign'>S</span>`)
      );
    }
    if (seconds >= 0) {
