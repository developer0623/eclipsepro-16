...
export class ParetoComponent implements OnChanges {
  ...
  calculateParetoVm() {
    ...
    let tempValues = this.data.map((o) => ({
      name: o[this.name],
      value: o[this.value],
      occurances: o[this.occurances],
      codeExempt: o.codeExempt,
    }));
    //console.log(tempValues);
    // Sort the array by descending value
    /*global _ */
    _.sortBy(tempValues, (o) => {
      return -1 * o.value;
    });
    ...
    if (other.count > 0) {
      vm.push({
        ...
        value: other.total,
        toolTipText: other.combinedNames,
      });
    }
    ...
    let sumValues = _.reduce(tempValues, (sum, val) => {
